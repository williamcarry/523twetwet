...
<template>
  <div>
    ...
    <div v-else class="bg-white rounded-b-lg border-b border-l border-r border-slate-200 overflow-hidden">
      ...
      <!-- 空状态 -->
      <div v-else-if="announcements.length === 0" class="p-10 text-center text-slate-500">
        {{ t('noAnnouncement') }}
      </div>
      ...
      <template v-else>
        <ul class="divide-y divide-slate-200">
          <li v-for="announcement in announcements" :key="announcement.id" class="p-5 hover:bg-slate-50 transition">
            <div class="flex announcement-item-wrapper">
              <div class="flex-1">
                <div class="flex items-start announcement-header mb-2">
                  <!-- 未读小圆点 -->
                  <div v-if="!announcement.isRead" class="w-2 h-2 bg-primary rounded-full mt-2 flex-shrink-0"></div>
                  <!-- 已读时的占位空间，保持布局一致 -->
                ...
                <div class="flex items-center justify-between ml-5">
                  <span class="text-xs text-slate-500">{{ formatDate(announcement.sendTime || announcement.createdAt) }}</span>
                  <button
                    @click="viewAnnouncement(announcement.id)"
...
const loadTranslations = async () => {
  try {
    const response = await fetch('/frondend/lang/MallAnnouncementPage.json')
...
const loadAnnouncements = async (page = 1) => {
  loading.value = true
  try {
    const response = await fetch(`/shop/api/message-center/mall-announcements?page=${page}`, {
      method: 'GET',
      headers: {
    ...
    if (result.success) {
      ...
      console.error('加载公告失败:', result.error)
    }
  } catch (error) {
...
/* 公告列表项内容区 - gap-4 替代 */
.announcement-item-wrapper > *:not(:last-child) {
  margin-right: 1rem; /* 16px */
}

/* 公告标题区 - gap-3 替代 */
.announcement-header > *:not(:last-child) {
