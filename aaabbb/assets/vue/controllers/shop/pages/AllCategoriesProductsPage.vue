    ...
    <div class="mx-auto w-full max-w-[1500px] md:w-[80%] md:min-w-[1150px] px-4 md:px-0 py-6">
      ...
      <div class="bg-white shadow-sm mb-6">
        ...
        <div class="flex items-center px-5 py-2 border-b border-slate-200">
          ...
          <div class="flex-1 flex flex-wrap items-center" style="gap: 0.5rem;">
            <span v-if="!selectedWarehouses && !selectedNewTime && !selectedShipment && !selectedSaleMode" class="text-slate-500 text-sm">{{ t('none') }}</span>
            <template v-else>
              <span v-if="selectedWarehouses" class="inline-flex items-center text-xs bg-slate-100 px-2 py-1 rounded" style="gap: 0.25rem;">{{ getWarehouseName(selectedWarehouses) }} <button @click="selectedWarehouses = null" class="text-slate-600 hover:text-primary font-bold">×</button></span>
              <span v-if="selectedNewTime" class="inline-flex items-center text-xs bg-slate-100 px-2 py-1 rounded" style="gap: 0.25rem;">{{ getNewTimeLabel(selectedNewTime) }} <button @click="selectedNewTime = ''" class="text-slate-600 hover:text-primary font-bold">×</button></span>
              <span v-if="selectedShipment" class="inline-flex items-center text-xs bg-slate-100 px-2 py-1 rounded" style="gap: 0.25rem;">{{ getShipmentLabel(selectedShipment) }} <button @click="selectedShipment = ''" class="text-slate-600 hover:text-primary font-bold">×</button></span>
              <span v-if="selectedSaleMode" class="inline-flex items-center text-xs bg-slate-100 px-2 py-1 rounded" style="gap: 0.25rem;">{{ getSaleModeLabel(selectedSaleMode) }} <button @click="selectedSaleMode = ''" class="text-slate-600 hover:text-primary font-bold">×</button></span>
            </template>
          </div>
        </div>
        ...
        <div class="flex items-center px-5 py-3 border-b border-slate-200">
          ...
          <div class="flex-shrink-0">
            <ul class="flex items-center batch-actions-list">
              ...
              <li class="batch-action-item"><a href="javascript:;" @click="handleBatchFavorite" class="text-xs text-slate-700 hover:text-primary transition inline-flex items-center action-link-item">
                <!-- 已收藏：实心红心，不设置 stroke -->
            ...
            <div class="p-4">
              ...
              <div class="mb-3 p-2 bg-slate-50 rounded text-center price-box">
                <template v-if="getCurrentDiscount(product) > 0">
                  ...
                  <div class="flex items-center justify-center price-display-wrapper">
              ...
              <!-- 区域选择器（价格下方，只显示英文简写） -->
              <div class="mb-3 flex items-center justify-center flex-wrap region-selector-wrapper" 
              ...
              <div class="flex items-center justify-between pt-3">
                <label class="flex items-center cursor-pointer product-checkbox-label">
                  <input 
                    type="checkbox" 
                    class="rounded" 
                ...
                <div class="flex items-center product-actions-list">
...
<style scoped>
/* 注意：复选框样式、批量操作按钮样式、商品操作样式已移至全局 browser-compat.css */
