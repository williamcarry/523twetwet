    ...
    >
      ...
      <div class="w-full pt-12 pb-8">
        <div class="mx-auto w-full max-w-[1500px] md:w-[80%] md:min-w-[1200px] px-4 md:px-0">
          <div class="text-center">
            <div class="flex items-center justify-center mb-4 hero-title">
              <svg class="w-8 h-8 text-white title-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="mx-auto w-full max-w-[1500px] md:w-[80%] md:min-w-[1200px] px-4 md:px-0 pb-8">
        <div class="bg-white rounded-lg p-5">
          ...
          <!-- 排行榜骨架屏 -->
          <div v-if="isLoadingRankings" class="flex ranking-skeleton">
            <div v-for="i in 4" :key="i" class="bg-gray-100 rounded-lg p-4 flex-1 skeleton-column">
              <!-- 标题骨架 -->
              <div class="flex items-center mb-4 skeleton-header">
                <div class="skeleton-box w-6 h-6 rounded skeleton-icon"></div>
                <div class="skeleton-box w-24 h-5"></div>
              </div>
              <!-- 列表项骨架（3个即可） -->
              <ul class="space-y-4">
                <li v-for="j in 3" :key="j" class="flex bg-white rounded p-3 skeleton-item">
                  <div class="skeleton-box w-[35%] aspect-square rounded skeleton-image"></div>
                  <div class="flex-1 flex flex-col skeleton-content">
                    <div class="skeleton-box w-full h-10 skeleton-text"></div>
                    <div class="skeleton-box w-3/4 h-4 skeleton-text"></div>
          ...
          <div v-else class="flex ranking-columns">
            <!-- Column 1: 热度排行榜 -->
            <div class="bg-gray-100 rounded-lg p-4 flex-1">
              <div class="flex items-center mb-4 ranking-header">
                <img
                  src="/frondend/images/HotSalesPage/topsales_icon1.png"
                  alt="热度"
                  class="w-6 h-6 ranking-icon"
                />
                <span class="font-bold text-base text-gray-800">热度排行榜</span>
              </div>
              <ul class="space-y-4 max-h-[490px] overflow-y-auto">
                <li v-for="(item, index) in hotRankings" :key="'hot-' + index" class="flex bg-white rounded p-3 ranking-item">
                  <div class="relative flex-shrink-0 w-[35%] ranking-image">
                  ...
                  <div class="flex-1 flex flex-col text-xs">
                    ...
                    <div class="flex items-center mb-1 ranking-stats">
                      <span class="flex items-center text-red-600 font-semibold stat-item">
                        <img src="/frondend/images/HotSalesPage/topsales_icon1.png" alt="热度" class="w-4 h-4 stat-icon" />
                        {{ item.heat }}
                      </span>
            ...
            <div class="bg-gray-100 rounded-lg p-4 flex-1">
              ...
              <ul class="space-y-4 max-h-[490px] overflow-y-auto">
                <li v-for="(item, index) in favoriteRankings" :key="'fav-' + index" class="flex bg-white rounded p-3 ranking-item">
                  <div class="relative flex-shrink-0 w-[35%] ranking-image">
                    <a :href="`/shop/item/${item.id}`" class="block">
                      <div class="relative w-full aspect-square bg-gray-100 rounded overflow-hidden">
                        <img
                  ...
                  <div class="flex-1 flex flex-col text-xs">
                    ...
                    <div class="flex items-center mb-1 ranking-stats">
                      <span class="flex items-center text-red-600 font-semibold stat-item">
                        <img src="/frondend/images/HotSalesPage/topsales_icon2.png" alt="收藏" class="w-4 h-4 stat-icon" />
                        {{ item.favorites }}
                      </span>
                      <span class="text-gray-500" v-if="item.stock">{{ t('stock') }}:{{ item.stock }}</span>
                    </div>
            ...
            <div class="bg-gray-100 rounded-lg p-4 flex-1">
              ...
              <ul class="space-y-4 max-h-[490px] overflow-y-auto">
                <li v-for="(item, index) in downloadRankings" :key="'down-' + index" class="flex bg-white rounded p-3 ranking-item">
                  <div class="relative flex-shrink-0 w-[35%] ranking-image">
                    <a :href="`/shop/item/${item.id}`" class="block">
                      <div class="relative w-full aspect-square bg-gray-100 rounded overflow-hidden">
                        <img
                  ...
                  <div class="flex-1 flex flex-col text-xs">
                    ...
                    <div class="flex items-center mb-1 ranking-stats">
                      <span class="flex items-center text-red-600 font-semibold stat-item">
                        <img src="/frondend/images/HotSalesPage/topsales_icon3.png" :alt="t('downloadRankings')" class="w-4 h-4 stat-icon" />
                        {{ item.downloads }}
                      </span>
                      <span class="text-gray-500" v-if="item.stock">{{ t('stock') }}:{{ item.stock }}</span>
                    </div>
            ...
            <div class="bg-gray-100 rounded-lg p-4 flex-1">
              ...
              <ul class="space-y-4 max-h-[490px] overflow-y-auto">
                <li v-for="(item, index) in publishRankings" :key="'pub-' + index" class="flex bg-white rounded p-3 ranking-item">
                  <div class="relative flex-shrink-0 w-[35%] ranking-image">
                    <a :href="`/shop/item/${item.id}`" class="block">
                      <div class="relative w-full aspect-square bg-gray-100 rounded overflow-hidden">
                        <img
                  ...
                  <div class="flex-1 flex flex-col text-xs">
                    ...
                    <div class="flex items-center mb-1 ranking-stats">
                      <span class="flex items-center text-red-600 font-semibold stat-item">
                        <img src="/frondend/images/HotSalesPage/topsales_icon4.png" alt="刊登" class="w-4 h-4 stat-icon" />
                        {{ item.publishes }}
                      </span>
                      <span class="text-gray-500" v-if="item.stock">{{ t('stock') }}:{{ item.stock }}</span>
                    </div>
      ...
      <div class="mx-auto w-full max-w-[1500px] md:w-[80%] md:min-w-[1200px] px-4 md:px-0 pb-8">
            ...
            >
              ...
              ">
                <div
                  ...
                  class="flex-shrink-0 w-[160px] flex flex-col items-center justify-center cursor-pointer text-center category-item px-7 py-5 relative"
                  @selectstart.prevent
                >
                  <img
                    :src="category.image || ''"
                    :alt="category.title"
                    class="w-20 h-20 rounded-full object-cover category-image"
                  />
        ...
        <!-- Filter Bar -->
        <div class="flex justify-between items-center py-1.5 filter-bar">
          ...
          <!-- 右侧操作按钮 -->
          <div class="flex items-center flex-shrink-0 action-buttons">
            <!-- 全选按钮 -->
            <button @click="handleSelectAll" class="flex items-center transition bg-white border border-primary text-primary px-3 py-1.5 rounded-full text-sm select-all-btn">
              <input 
                type="checkbox" 
                v-model="selectAll" 
                @click.stop="handleSelectAll"
                class="rounded cursor-pointer pointer-events-none select-checkbox"
              />
            ...
            <!-- 批量下载按钮 -->
            <button @click="batchDownloadProducts" :disabled="batchDownloading" :class="{'opacity-50 cursor-not-allowed': batchDownloading}" class="flex items-center transition bg-white border border-primary text-primary px-3 py-1.5 rounded-full text-sm download-btn">
              <svg v-if="batchDownloading" class="w-4 h-4 animate-spin download-icon" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>
              <svg v-else xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#cb261c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="flex-shrink-0 download-icon">
        ...
        <div v-else>
          <ul
            ...
            :key="groupIndex"
            class="flex flex-wrap mb-2.5 list-none product-row"
          >
            <li
              ...
              <div class="p-2 flex flex-col flex-1">
                ...
                <div class="mb-2 p-2 bg-slate-50 rounded text-center">
                  <template v-if="getCurrentDiscount(product) > 0">
                    <!-- 有折扣：显示折扣价（红色）+ 原价（删除线） -->
                    <div class="flex items-center justify-center price-with-discount">
                      <span class="text-red-500 font-bold text-lg discount-price">
                        {{ formatPrice(getCurrentPriceInfo(product).sellingPrice, getCurrentPriceInfo(product).currency) }}
                      </span>
                      <span class="text-gray-400 line-through text-sm original-price">
                ...
                <!-- 区域选择器（价格下方，只显示英文简写） -->
                <div class="mb-2 flex items-center justify-center flex-wrap region-selector" 
...
/* 标题区域 */
.hero-title > .title-icon:not(:last-child) {
  margin-right: 12px;
}

/* 排行榜骨架屏 */
.ranking-skeleton > .skeleton-column:not(:last-child) {
  margin-right: 16px;
}

.skeleton-header > .skeleton-icon:not(:last-child) {
  margin-right: 8px;
}

.skeleton-item > .skeleton-image:not(:last-child) {
  margin-right: 8px;
}

.skeleton-content > .skeleton-text:not(:last-child) {
  margin-bottom: 8px;
}

/* 排行榜列 */
.ranking-columns > *:not(:last-child) {
  margin-right: 16px;
}

.ranking-header > .ranking-icon:not(:last-child) {
  margin-right: 8px;
}

.ranking-item > .ranking-image:not(:last-child) {
  margin-right: 8px;
}

.ranking-stats > .stat-item:not(:last-child) {
  margin-right: 12px;
}

.stat-item > .stat-icon:not(:last-child) {
  margin-right: 4px;
}

/* 分类轮播 */
.category-item > .category-image:not(:last-child) {
  margin-bottom: 8px;
}
...
.select-all-btn > .select-checkbox:not(:last-child) {
  margin-right: 8px;
}

.download-btn > .download-icon:not(:last-child) {
  margin-right: 8px;
}
...
/* 价格显示 */
.price-with-discount > .discount-price:not(:last-child) {
  margin-right: 8px;
}
