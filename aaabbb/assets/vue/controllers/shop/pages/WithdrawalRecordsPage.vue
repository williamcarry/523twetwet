  ...
  <div class="bg-white">
    <main class="p-6">
      ...
      <div class="flex items-center justify-between mb-6">
        ...
        <!-- 状态筛选和刷新按钮 -->
        <div class="flex items-center withdrawal-filter-controls">
      ...
      <!-- 提现记录列表 -->
      <div v-else-if="withdrawalList.length > 0" class="space-y-3">
        <div 
          v-for="record in withdrawalList" 
          :key="record.id"
        ...
        >
          <div class="p-4">
            ...
            <div class="flex items-center justify-between mb-3 pb-3 border-b border-gray-200">
              <div class="flex items-center withdrawal-order-info">
                <span class="text-sm text-gray-500">{{ t('orderNo') }}:</span>
                <span class="font-mono text-sm font-medium text-gray-900" style="margin-left: 0.5rem;">{{ record.orderNo }}</span>
              </div>
              <span 
            ...
            <!-- 主要信息网格 -->
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 mb-3 withdrawal-info-grid">
              <!-- 提现金额 -->
              <div>
              ...
              <!-- 提现方式 -->
              <div>
                <div class="text-sm text-gray-500 mb-0.5">{{ t('withdrawalMethod') }}</div>
                <div class="text-sm text-gray-900">{{ getWithdrawalMethodText(record.withdrawalMethod) }}</div>
              </div>
              ...
              <!-- 审核时间 -->
              <div>
                <div class="text-sm text-gray-500 mb-0.5">{{ t('reviewedAt') }}</div>
                <div class="text-sm text-gray-900">
                  {{ record.reviewedAt || t('notReviewed') }}
                </div>
              </div>
            </div>

            <!-- 收款信息 -->
            <div v-if="record.paymentInfo" class="mb-3">
              <span class="text-sm text-gray-500">{{ t('paymentInfo') }}:</span>
              <span class="text-sm text-gray-700 ml-2">{{ record.paymentInfo }}</span>
            </div>

            <!-- 备注信息 -->
            <div v-if="record.remark" class="mb-3">
              <span class="text-sm text-gray-500">{{ t('remark') }}:</span>
              <span class="text-sm text-gray-700 ml-2">{{ record.remark }}</span>
            </div>

            <!-- 底部附加信息 -->
            <div class="flex flex-wrap text-sm text-gray-500 withdrawal-extra-info">
...
const loadTranslations = async () => {
  try {
    const response = await fetch('/frondend/lang/WithdrawalRecordsPage.json')
...
const getWithdrawalMethodText = (method) => {
  const methodKey = `withdrawalMethod${method.charAt(0).toUpperCase() + method.slice(1)}`
...
const fetchWithdrawalList = async () => {
  ...
  try {
    ...
    const response = await fetch('/shop/api/withdrawal/list', {
      method: 'POST',
      headers: {
    ...
    if (result.success) {
      withdrawalList.value = result.data.list
...
/* 确保筛选控件间距 */
.withdrawal-filter-controls > label {
  margin-right: 1rem;
}

.withdrawal-filter-controls > select {
  margin-left: 0;
}

/* 确保按钮内图标和文字间距 */
.withdrawal-filter-controls button svg {
  margin-right: 0.5rem;
}
</style>
