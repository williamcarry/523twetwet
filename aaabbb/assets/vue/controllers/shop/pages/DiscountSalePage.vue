  ...
  <div class="min-h-screen flex flex-col">
    ...
    <div class="w-full" style="padding: 40px 20px;">
      <div class="mx-auto px-4" style="width: 100%; max-width: 1500px; margin: 0 auto;">
        <div class="flex items-center justify-center" style="margin: -12px;">
          <div style="padding: 12px; display: flex; align-items: center; width: 100%; justify-content: center;">
          <img loading="lazy" src="/frondend/images/DiscountSalePage/promotion_icon.png" alt="promotion" style="width: 48px; height: 48px; margin-right: 24px;" />
            ...
            <li v-for="(p, index) in recommendedTop" :key="`top-${p.id}-${index}`" class="promo-item">
              ...
              <div class="promo-info">
                ...
                <div class="promo-meta">
                  <!-- 价格显示 -->
                  <div v-if="getCurrentDiscount(p) > 0" class="flex items-center" style="margin: -4px;">
                    <div style="padding: 4px; display: flex; align-items: center; width: 100%;">
                    <span class="price" style="margin-right: 8px;">{{ getCurrentCurrency(p) }} {{ calculateDiscountedPrice(p).toFixed(2) }}</span>
                ...
                <!-- 区域选择器 -->
                <div class="flex items-center justify-start flex-wrap mt-2" style="margin: -2px;" v-if="p.shippingRegions && p.shippingRegions.length > 0">
                  <button
              ...
              <li v-for="(p, i) in row" :key="p.id + '-l-' + rIdx + '-' + i" class="product-card">
                ...
                <div class="product-content">
                  ...
                  <div class="product-price">
                    <!-- 价格显示 -->
                    <div v-if="getCurrentDiscount(p) > 0" class="flex flex-col" style="margin: -2px;">
                      <div style="padding: 2px; display: flex; flex-direction: column; width: 100%;">
                  ...
                  <!-- 区域选择器 -->
                  <div class="flex items-center justify-start flex-wrap mb-2" style="margin: -2px;" v-if="p.shippingRegions && p.shippingRegions.length > 0">
                    <button
                      v-for="region in p.shippingRegions"
                      :key="region"
                      @click.prevent="selectRegion(p, region)"
                  ...
                  <div class="flex items-center justify-end pt-3">
                    <!-- 隐藏复选框但保留代码 -->
                    <label class="flex items-center cursor-pointer" style="margin-right: 8px; display: none;">
                      <input 
                        type="checkbox" 
                        class="rounded"
                        :value="p.spu"
                    ...
                    <div class="flex items-center" style="margin: -4px;">
                      <button :title="t('oneClickPublish')" class="p-2 text-slate-600 hover:text-primary transition" style="margin: 4px;">
                        <img src="/frondend/images/DiscountSalePage/flasting_logo.png" alt="一键刊登" class="w-4 h-4" />
                      </button>
                      <button :title="t('download')" @click="downloadProduct(p.id)" class="p-2 text-slate-600 hover:text-primary transition" style="margin: 4px;">
                        <!-- 已下载：实心红色 -->
                      ...
                      <button :title="t('collect')" class="p-2 text-slate-600 hover:text-primary transition" @click="toggleFavorite(p.id)" style="margin: 4px;">
                        <!-- 已收藏：实心红心，不设置 stroke -->
    ...
    <div class="mx-auto px-4 py-4" style="width: 100%; max-width: 1500px; margin: 0 auto;">
      <div class="grid grid-cols-1 lg:grid-cols-5" style="margin: -8px;">
        <div style="margin: 8px;" class="lg:col-span-1">
        ...
        <aside>
          <div class="bg-white rounded-sm sticky top-20 max-h-[70vh] overflow-y-auto p-4" style="padding: 12px 10px;">
            ...
            <div class="mb-4">
              ...
              <ul class="list-none p-0 m-0 flex flex-col" style="margin: -4px;">
                <!-- 一级分类 -->
                <li class="flex relative" style="margin: 4px;">
            ...
            <div class="mb-4">
              ...
              <ul class="list-none p-0 m-0 flex flex-col" style="margin: -4px;">
                <li class="flex items-center mb-2 cursor-pointer" @click="discountRange = 'all'" style="margin: 4px;">
                  <input type="radio" name="DiscountStrength" value="all" v-model="discountRange" class="hidden" />
                  <div class="flex items-center cursor-pointer user-select-none" style="margin: -4px;">
                    <div style="margin: 4px;"><i :class="['radio-icon w-[18px] h-[18px] border-2 rounded-full bg-white cursor-pointer flex items-center justify-center relative flex-shrink-0', discountRange === 'all' ? 'border-red-700 bg-red-700' : 'border-gray-400']"></i></div>
                    <div class="text-gray-800 text-sm cursor-pointer user-select-none" style="margin: 4px;">{{ t('all') }}</div>
                  </div>
                </li>
                <li class="flex items-center mb-2 cursor-pointer" @click="discountRange = '>=1'" style="margin: 4px;">
                  <input type="radio" name="DiscountStrength" value=">=1" v-model="discountRange" class="hidden" />
                  <div class="flex items-center cursor-pointer user-select-none" style="margin: -4px;">
                    <div style="margin: 4px;"><i :class="['radio-icon w-[18px] h-[18px] border-2 rounded-full bg-white cursor-pointer flex items-center justify-center relative flex-shrink-0', discountRange === '>=1' ? 'border-red-700 bg-red-700' : 'border-gray-400']"></i></div>
                    <div class="text-gray-800 text-sm cursor-pointer user-select-none" style="margin: 4px;">1%-10%</div>
                  </div>
                </li>
                <li class="flex items-center mb-2 cursor-pointer" @click="discountRange = '>=2'" style="margin: 4px;">
                  <input type="radio" name="DiscountStrength" value=">=2" v-model="discountRange" class="hidden" />
                  <div class="flex items-center cursor-pointer user-select-none" style="margin: -4px;">
                    <div style="margin: 4px;"><i :class="['radio-icon w-[18px] h-[18px] border-2 rounded-full bg-white cursor-pointer flex items-center justify-center relative flex-shrink-0', discountRange === '>=2' ? 'border-red-700 bg-red-700' : 'border-gray-400']"></i></div>
                    <div class="text-gray-800 text-sm cursor-pointer user-select-none" style="margin: 4px;">10%-20%</div>
                  </div>
                </li>
                <li class="flex items-center mb-2 cursor-pointer" @click="discountRange = '>=3'" style="margin: 4px;">
                  <input type="radio" name="DiscountStrength" value=">=3" v-model="discountRange" class="hidden" />
                  <div class="flex items-center cursor-pointer user-select-none" style="margin: -4px;">
                    <div style="margin: 4px;"><i :class="['radio-icon w-[18px] h-[18px] border-2 rounded-full bg-white cursor-pointer flex items-center justify-center relative flex-shrink-0', discountRange === '>=3' ? 'border-red-700 bg-red-700' : 'border-gray-400']"></i></div>
                    <div class="text-gray-800 text-sm cursor-pointer user-select-none" style="margin: 4px;">20%-50%</div>
                  </div>
                </li>
                <li class="flex items-center mb-2 cursor-pointer" @click="discountRange = '>=4'" style="margin: 4px;">
                  <input type="radio" name="DiscountStrength" value=">=4" v-model="discountRange" class="hidden" />
                  <div class="flex items-center cursor-pointer user-select-none" style="margin: -4px;">
                    <div style="margin: 4px;"><i :class="['radio-icon w-[18px] h-[18px] border-2 rounded-full bg-white cursor-pointer flex items-center justify-center relative flex-shrink-0', discountRange === '>=4' ? 'border-red-700 bg-red-700' : 'border-gray-400']"></i></div>
                    <div class="text-gray-800 text-sm cursor-pointer user-select-none" style="margin: 4px;">50%+</div>
                  </div>
                </li>
              </ul>
            </div>
            ...
            <div class="mb-4">
              ...
              <div class="flex items-center w-full">
                <div class="flex items-center flex-1" style="margin: -2px;">
                  <input type="text" v-model="minPrice" :placeholder="t('minPrice')" class="bg-white border border-gray-200 rounded px-2 h-[34px] text-gray-800 text-xs w-[40%] overflow-hidden text-ellipsis focus:outline-none focus:border-gray-200" style="margin: 2px;" />
                  <span class="text-gray-800 text-sm user-select-none flex-shrink-0" style="margin: 2px;">-</span>
                  <input type="text" v-model="maxPrice" :placeholder="t('maxPrice')" class="bg-white border border-gray-200 rounded px-2 h-[34px] text-gray-800 text-xs w-[40%] overflow-hidden text-ellipsis focus:outline-none focus:border-gray-200" style="margin: 2px;" />
                </div>
              </div>
            </div>
            ...
            <div class="mb-0">
              <button type="button" @click="applyFilters" :disabled="isFiltering" class="w-full bg-red-700 border-none rounded px-4 h-[38px] text-white text-sm font-semibold cursor-pointer user-select-none transition-opacity duration-300 hover:opacity-90 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center" style="margin: -4px;">
                <div style="padding: 4px; display: flex; align-items: center; justify-content: center; width: 100%;">
                <svg v-if="isFiltering" class="animate-spin h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" style="margin-right: 8px;">
        ...
        <div style="margin: 8px;" class="lg:col-span-4">
        <!-- Right side: Product grid and toolbar -->
        <main>
          <!-- Toolbar -->
          <div class="bg-white rounded-t flex justify-between items-center mb-0 p-2 w-full">
            <div>
              <ul class="flex items-center flex-wrap" style="margin: -8px;">
                <li class="flex items-center h-[32px] relative z-30" style="margin: 8px;">
                ...
                <li class="h-[32px] leading-[32px] text-sm" style="margin: 8px;">
                  <p>{{ t('selected') }} <em class="text-red-600">{{ selectedCount }}</em> {{ t('itemsCount') }}</p>
                </li>
                <li class="h-[32px] leading-[32px]" style="margin: 8px;">
                  <a href="javascript:;" @click="publishAll" class="flex items-center cursor-pointer transition-colors duration-300 text-gray-500 hover:text-red-700" style="margin: -4px;">
                    <img src="/frondend/images/DiscountSalePage/publish_icon.png" class="w-4 h-4 flex-shrink-0" style="margin: 4px;" />
                    <span style="margin: 4px;">{{ t('oneClickPublish') }}</span>
                  </a>
                </li>
                <li class="h-[32px] leading-[32px]" style="margin: 8px;">
                  <a href="javascript:;" @click="batchDownloadProducts" :class="{'opacity-50 cursor-not-allowed': batchDownloading}" class="flex items-center cursor-pointer transition-colors duration-300 text-gray-500 hover:text-red-700" style="margin: -4px;">
                    <svg v-if="batchDownloading" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24" style="margin: 4px;">
                      <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                      <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    <img v-else src="/frondend/images/DiscountSalePage/download_icon.png" class="w-4 h-4 flex-shrink-0" style="margin: 4px;" />
                    <span style="margin: 4px;">{{ batchDownloading ? (currentLang === 'en' ? 'Processing...' : '处理中...') : t('downloadPackage') }}</span>
                  </a>
                </li>
                <li class="h-[32px] leading-[32px]" style="margin: 8px;">
                  <a href="javascript:;" @click="batchAddToFavorites" class="flex items-center cursor-pointer transition-colors duration-300 text-gray-500 hover:text-red-700" style="margin: -4px;">
                    <HeartIcon :size="16" class="flex-shrink-0" style="margin: 4px;" />
                    <span style="margin: 4px;">{{ t('addToFavorites') }}</span>
                  </a>
                </li>
              </ul>
            </div>
            <div class="flex items-center h-[32px]" style="margin: -4px;">
              <p class="text-sm" style="margin: 4px;">{{ t('sort') }}</p>
              <select v-model="sortBy" class="border border-gray-300 rounded bg-white px-3 h-[38px] text-sm cursor-pointer" style="margin: 4px;">
            ...
            <div v-else class="grid-products">
              <div v-for="p in pagedProducts" :key="p.spu" class="product-card-grid">
                ...
                <div class="product-content">
                  ...
                  <!-- 区域选择器 -->
                  <div class="flex items-center justify-center flex-wrap mb-2" style="margin: -2px;" v-if="p.shippingRegions && p.shippingRegions.length > 0">
                    <button
                      v-for="region in p.shippingRegions"
                      :key="region"
                      @click.prevent="selectRegion(p, region)"
                  ...
                  <div class="flex items-center justify-between pt-3">
                    <label class="flex items-center cursor-pointer" style="margin-right: 8px;">
                      <input 
                        type="checkbox" 
                        class="rounded"
                        :value="p.spu"
...
.promo-item {
  width: calc(25% - 12px);  /* 4列布局，每个占 25%，减去自身的 margin-right */
  height: 136.55px;
  background: #f6f6f6;
  border-radius: 3px;
  padding: 8px;
...
.product-card-grid {
  position: relative;
  box-sizing: border-box;
  width: calc(20% - 12px);  /* 5列布局，每个占 20%，减去自身的 margin-right */
  background: #fff;
  border-radius: 3px;
  overflow: hidden;
  display: flex;
