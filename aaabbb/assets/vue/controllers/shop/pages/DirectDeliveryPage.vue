  ...
  <div class="min-h-screen" style="background-color: #f2f3f7">
      ...
      <div v-else class="bg-transparent mx-auto w-full max-w-[1500px]">
        ...
        <div class="flex flex-wrap -mx-2 mb-4">
          <div v-for="(p, idx) in hotRecommend.slice(0,2)" :key="p.spu" class="w-full md:w-1/2 px-2 mb-4 md:mb-0">
            <div class="bg-white p-5 flex items-start relative">
              <!-- 折扣标签（整个区域的右上角） -->
              ...
              <a :href="`/shop/item/${p.id}`" target="_blank" class="relative block flex-shrink-0 w-1/3 mr-4">
              ...
              <div class="flex-1">
                ...
                <!-- 价格显示 -->
                <div class="mt-2 flex items-center" style="margin: -6px;">
                  <!-- 有折扣时显示折扣价 -->
                  <template v-if="getCurrentDiscount(p) > 0">
                    <b class="text-base font-bold text-red-600" style="margin: 6px;">{{ formatPrice(calculateDiscountedPrice(p), getCurrentCurrency(p)) }}</b>
                    <em class="text-sm text-slate-400 line-through" style="margin: 6px;">{{ formatPrice(getCurrentSellingPrice(p), getCurrentCurrency(p)) }}</em>
                  </template>
                  <!-- 无折扣时只显示售价 -->
                  <template v-else>
                    <b class="text-base font-bold text-red-600" style="margin: 6px;">{{ formatPrice(getCurrentSellingPrice(p), getCurrentCurrency(p)) }}</b>
                  </template>
                  <span class="ml-2 bg-orange-400 text-white text-xs px-2 py-0.5 rounded" style="margin: 6px;">{{ t('hotLabel') }}</span>
                </div>
                
                <!-- 区域选择器 -->
                <div class="mt-3 flex items-center flex-wrap" style="margin: -4px;" v-if="p.shippingRegions && p.shippingRegions.length > 0">
                  <button
                    ...
                    :style="getSelectedRegion(p) === region ? 'background-color: #CB261C; color: #ffffff; border-color: #CB261C; margin: 4px;' : 'margin: 4px;'"
                  >
                    {{ region }}
                  </button>
                </div>
        ...
        <div class="flex flex-wrap -mx-2">
          <div v-for="(p, idx) in hotRecommend.slice(2,5)" :key="p.spu" class="w-full md:w-1/3 px-2 mb-4">
            <div class="bg-white p-4">
              ...
              <!-- 价格显示 -->
              <div class="mt-2 flex items-center" style="margin: -4px;">
                <!-- 有折扣时显示折扣价 -->
                <template v-if="getCurrentDiscount(p) > 0">
                  <b class="text-sm text-red-600 font-bold" style="margin: 4px;">{{ formatPrice(calculateDiscountedPrice(p), getCurrentCurrency(p)) }}</b>
                  <span class="text-xs text-slate-400 line-through" style="margin: 4px;">{{ formatPrice(getCurrentSellingPrice(p), getCurrentCurrency(p)) }}</span>
                </template>
                <!-- 无折扣时只显示售价 -->
                <template v-else>
                  <b class="text-sm text-red-600 font-bold" style="margin: 4px;">{{ formatPrice(getCurrentSellingPrice(p), getCurrentCurrency(p)) }}</b>
                </template>
              </div>
              
              <!-- 区域选择器 -->
              <div class="mt-3 flex items-center flex-wrap justify-center" style="margin: -4px;" v-if="p.shippingRegions && p.shippingRegions.length > 0">
                <button
                  v-for="region in p.shippingRegions"
                  :key="region"
                  @click="selectRegion(p, region)"
    ...
    <section class="mx-auto w-full max-w-[1500px] md:w-[80%] md:min-w-[1200px] px-4 md:px-0 py-6">
      ...
      <div class="bg-white rounded-lg p-4 mb-6 relative">
        ...
        <div ref="catContainer" class="flex overflow-x-auto no-scrollbar py-2 px-10 scroll-smooth" style="margin: -6px;">
          <button
      ...
      <div v-else class="flex flex-wrap -mx-2">
        <div v-for="p in products" :key="p.spu" class="w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/5 px-2 mb-4">
          <div class="bg-white relative">
            <a :href="`/shop/item/${p.id}`" target="_blank" class="block">
              <div class="aspect-[1/1] bg-gray-100 overflow-hidden relative">
                <img :src="p.mainImage" :alt="p.title" class="w-full h-full object-cover" />
                <!-- 折扣标签（右上角） -->
            ...
            <div class="px-3 pb-3">
            <a :href="`/shop/item/${p.id}`" target="_blank" class="text-sm font-medium text-slate-800 hover:text-primary two-line mt-2 block">{{ getProductTitle(p) }}</a>
            
            <!-- 价格显示 -->
            <div class="mt-1 flex items-center" style="margin: -4px;">
              <!-- 有折扣时显示折扣价 -->
              <template v-if="getCurrentDiscount(p) > 0">
                <b class="text-sm text-red-600 font-bold" style="margin: 4px;">{{ formatPrice(calculateDiscountedPrice(p), getCurrentCurrency(p)) }}</b>
                <span class="text-xs text-slate-400 line-through" style="margin: 4px;">{{ formatPrice(getCurrentSellingPrice(p), getCurrentCurrency(p)) }}</span>
            ...
            <!-- 区域选择器（价格下方） -->
            <div class="mt-1 flex items-center justify-center flex-wrap" style="margin: -2px;" v-if="p.shippingRegions && p.shippingRegions.length > 0">
              <button
                v-for="region in p.shippingRegions"
                :key="region"
                @click="selectRegion(p, region)"
            ...
            <!-- 操作按钮区域 -->
            <div class="mt-3 flex items-center justify-end" style="margin: -4px;">
              <a :title="t('publishAction')" href="javascript:void(0)" class="action-publish" @click.prevent="onPublish(p)" :style="{ backgroundImage: 'url(/frondend/images/DirectDeliveryPage/3f8d8e905d1cb80e9c744fe7b9ad292a.png)' }" style="margin: 4px;"></a>
              <a :title="t('downloadAction')" href="javascript:void(0)" @click="downloadProduct(p.id)" class="action-download" :aria-label="t('downloadAction')" style="margin: 4px;">
                <!-- 已下载：实心红色 -->
              ...
              <a :title="t('favoriteAction')" href="javascript:void(0)" class="action-fav" @click.prevent="toggleFavorite(p.id)" style="margin: 4px;">
