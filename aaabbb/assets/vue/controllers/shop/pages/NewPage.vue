...
<template>
  <div class="min-h-screen">
    ...
    <div class="w-full" style="padding: 40px 20px;">
      <div class="mx-auto px-4" style="width: 100%; max-width: 1500px; margin: 0 auto;">
        <div class="flex items-center justify-center" style="margin: -12px;">
          <div style="padding: 12px; display: flex; align-items: center; justify-content: center; width: 100%;">
          <img loading="lazy" src="/frondend/images/newPage/new-icon.png" alt="new" style="width: 48px; height: 48px; margin-right: 24px;" />
    ...
    <div style="width: 100%; max-width: 1500px; margin: 0 auto; padding: 24px 20px;">
      <div class="bg-white rounded-lg shadow-md" style="padding: 24px;">
        <div class="flex" style="margin: -12px;">
        <!-- Left Column: Weekly Featured (本周强推) -->
        <div class="w-[35%]" style="margin: 12px; margin-right: 24px;">
          ...
          <div v-else-if="weeklyFeatured.length > 0" class="mt-4">
            ...
            <!-- 区域选择器 -->
            <div class="flex items-center justify-start flex-wrap mt-2" style="margin: -2px;" v-if="weeklyFeatured[currentWeeklyIdx].shippingRegions && weeklyFeatured[currentWeeklyIdx].shippingRegions.length > 0">
              <button
        ...
        <div style="flex: 1; margin: 12px;">
          <div class="flex items-center justify-between bg-white p-4 rounded-lg mb-4">
            ...
            <ul v-if="!apiLoading" class="flex flex-1 justify-end flex-wrap" style="align-content: flex-start; margin: -8px;">
              <li
                ...
                style="max-width: 150px; min-height: 36px; display: flex; align-items: center; justify-content: center; word-break: break-word; margin: 8px;"
              >
          ...
          <!-- Product Grid -->
          <div v-else class="flex flex-wrap" style="margin: -8px;">
            <div v-for="(p, index) in hotSalesTop8" :key="`hot-sales-cat${selectedHotSalesCategory}-${p.id}-${index}`" class="bg-[#f6f6f6] rounded-lg overflow-hidden hover:shadow transition" style="width: calc(25% - 16px); margin: 8px;">
    ...
    <div style="width: 100%; max-width: 1500px; margin: 0 auto; padding: 24px 20px;">
      <div class="grid grid-cols-1 lg:grid-cols-5" style="margin: -8px;">
        ...
        <aside class="lg:col-span-1" style="margin: 8px;">
          ...
          <div class="mb-4">
            ...
            <div class="flex items-center w-full">
              <div class="flex items-center flex-1" style="margin: -2px;">
                <input v-model.number="inventoryStart" type="text" :placeholder="t('minPrice')" class="bg-white border border-gray-200 rounded px-2 h-[34px] text-gray-800 text-xs w-[40%] overflow-hidden text-ellipsis focus:outline-none focus:border-gray-200" style="margin: 2px;" />
                <span class="text-gray-800 text-sm user-select-none flex-shrink-0" style="margin: 2px;">-</span>
                <input v-model.number="inventoryEnd" type="text" :placeholder="t('maxPrice')" class="bg-white border border-gray-200 rounded px-2 h-[34px] text-gray-800 text-xs w-[40%] overflow-hidden text-ellipsis focus:outline-none focus:border-gray-200" style="margin: 2px;" />
              </div>
          ...
          <div class="mb-4">
            ...
            <div class="flex items-center w-full">
              <div class="flex items-center flex-1" style="margin: -2px;">
                <input v-model.number="priceStart" type="text" :placeholder="t('minPrice')" class="bg-white border border-gray-200 rounded px-2 h-[34px] text-gray-800 text-xs w-[40%] overflow-hidden text-ellipsis focus:outline-none focus:border-gray-200" style="margin: 2px;" />
                <span class="text-gray-800 text-sm user-select-none flex-shrink-0" style="margin: 2px;">-</span>
                <input v-model.number="priceEnd" type="text" :placeholder="t('maxPrice')" class="bg-white border border-gray-200 rounded px-2 h-[34px] text-gray-800 text-xs w-[40%] overflow-hidden text-ellipsis focus:outline-none focus:border-gray-200" style="margin: 2px;" />
              </div>
        ...
        <!-- Right Content Area -->
        <main class="lg:col-span-4" style="margin: 8px;">
          <!-- Category Carousel -->
              ...
              >
                ...
                >
                  <div
                    ...
                    class="flex-shrink-0 w-[160px] flex flex-col items-center justify-center cursor-pointer text-center px-7 py-5 relative"
                    style="margin: -4px;"
                    @selectstart.prevent
                  >
                    <div style="padding: 4px; display: flex; flex-direction: column; align-items: center; justify-content: center; width: 100%;">
                    <img
                      :src="category.image"
          ...
          <div class="bg-white rounded flex justify-between items-center mb-4 p-3 w-full">
            <div>
              ...
              <li class="h-[32px] leading-[32px]" style="margin: 8px;">
                <a href="javascript:;" @click="publishAll" class="flex items-center cursor-pointer transition-colors duration-300 text-gray-500 hover:text-red-700" style="margin: -4px;">
                  <img src="https://www.saleyee.com/static/imgs/aa83cfdd3cebd0d28376adfbee407bf3.png" class="w-4 h-4 flex-shrink-0" :alt="t('oneClickPublish')" style="margin: 4px;" />
              ...
              <li class="h-[32px] leading-[32px]" style="margin: 8px;">
                <a href="javascript:;" @click="batchDownloadProducts" :class="{'opacity-50 cursor-not-allowed': batchDownloading}" class="flex items-center cursor-pointer transition-colors duration-300 text-gray-500 hover:text-red-700" style="margin: -4px;">
                  ...
                  <svg v-else xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#cb261c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="flex-shrink-0" style="margin: 4px;">
              ...
              <li class="h-[32px] leading-[32px]" style="margin: 8px;">
                <a href="javascript:;" @click="addToFavorites" class="flex items-center cursor-pointer transition-colors duration-300 text-gray-500 hover:text-red-700" style="margin: -4px;">
                  <HeartIcon :size="16" fill="none" stroke="currentColor" :stroke-width="2" class="flex-shrink-0" style="margin: 4px;" />
            ...
            <div class="flex items-center h-[32px]" style="margin: -4px;">
              <span class="text-sm text-slate-600" style="margin: 4px;">{{ t('sort') }}</span>
              <select
                v-model="sortBy"
              ...
              <div class="product-content">
                ...
                <div class="flex items-center justify-start flex-wrap mb-2" style="margin: -2px;" v-if="p.shippingRegions && p.shippingRegions.length > 0">
                  <button
                    ...
                    :style="getSelectedRegion(p) === region ? 'background-color: #CB261C; color: #ffffff; border-color: #CB261C; margin: 2px;' : 'margin: 2px;'"
                  >
                    {{ region }}
                  </button>
                </div>
                ...
                <div class="product-actions-bottom">
                  ...
                  <!-- 右侧：一键刊登、下载、收藏按钮 -->
                  <div class="flex items-center" style="margin-left: auto; margin: -4px;">
                    <a :title="t('oneClickPublish')" href="javascript:;" class="action-icon action-publish-small" style="margin: 4px;">
                      <img loading="lazy" src="/frondend/images/newPage/publish-icon.png" :alt="t('oneClickPublish')" />
                    </a>
                    <a :title="t('download')" href="javascript:;" @click="downloadProduct(p.id)" class="action-icon action-download-small" style="margin: 4px;">
                      <!-- 已下载：实心红色 -->
                    ...
                    <a :title="t('favorite')" href="javascript:;" @click.prevent="toggleFavorite(p.id)" class="action-icon action-collect-small" style="margin: 4px;">
...
const onCarouselMouseMove2 = (e) => {
  if (!isDragging2.value || !carouselContentRef2.value) return
  
  // 获取轮播内容容器的实际可视宽度
  const containerWidth = carouselContentRef2.value.clientWidth
  if (containerWidth === 0) return
  
  // 每个分类项宽度为 160px
  const itemWidth = 160
  const totalItems = categories.value.length
  const contentWidth = totalItems * itemWidth
  ...
  // 限制偏移量在 0 到 maxOffset 之间
  if (newOffset < 0) {
    carouselOffset2.value = 0
  } else if (newOffset > maxOffset) {
    carouselOffset2.value = maxOffset
  } else {
    carouselOffset2.value = newOffset
  }
}
...
const canScrollRight2 = computed(() => {
  if (!carouselContentRef2.value || categories.value.length === 0) return false
  ...
  // 当前偏移量必须小于最大偏移量才能继续滚动
  // 使用1px容差避免浮点数精度问题
  return carouselOffset2.value < (maxOffset - 1)
})

const scrollCarouselRight2 = () => {
  if (!carouselContentRef2.value || categories.value.length === 0) return
  ...
  // 每次滚动一个项目的宽度，但不超过最大偏移量
  const newOffset = carouselOffset2.value + itemWidth
  carouselOffset2.value = newOffset > maxOffset ? maxOffset : newOffset
}
