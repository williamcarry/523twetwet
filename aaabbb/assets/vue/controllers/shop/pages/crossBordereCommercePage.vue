    ...
    <div class="mx-auto w-full max-w-[1500px] md:w-[80%] md:min-w-[1150px] px-4 md:px-0">
      ...
      <div class="bg-white shadow-sm mb-6">
        ...
        <div class="flex items-center px-5 py-2 border-b border-slate-200">
          ...
          <div class="flex-1 flex flex-wrap items-center selected-conditions">
            <span v-if="!selectedWarehouses && !selectedNewTime && !selectedShipment && !selectedSaleMode" class="text-slate-500 text-sm">{{ t('none') }}</span>
            <template v-else>
              <span v-if="selectedWarehouses" class="inline-flex items-center condition-tag text-xs bg-slate-100 px-2 py-1 rounded">{{ getWarehouseName(selectedWarehouses) }} <button @click="selectedWarehouses = null" class="text-slate-600 hover:text-primary font-bold tag-close">×</button></span>
              <span v-if="selectedNewTime" class="inline-flex items-center condition-tag text-xs bg-slate-100 px-2 py-1 rounded">{{ getNewTimeLabel(selectedNewTime) }} <button @click="selectedNewTime = ''" class="text-slate-600 hover:text-primary font-bold tag-close">×</button></span>
              <span v-if="selectedShipment" class="inline-flex items-center condition-tag text-xs bg-slate-100 px-2 py-1 rounded">{{ getShipmentLabel(selectedShipment) }} <button @click="selectedShipment = ''" class="text-slate-600 hover:text-primary font-bold tag-close">×</button></span>
              <span v-if="selectedSaleMode" class="inline-flex items-center condition-tag text-xs bg-slate-100 px-2 py-1 rounded">{{ getSaleModeLabel(selectedSaleMode) }} <button @click="selectedSaleMode = ''" class="text-slate-600 hover:text-primary font-bold tag-close">×</button></span>
            </template>
          </div>
        </div>
        ...
        <div class="flex items-center px-5 py-2 border-b border-slate-200">
          ...
          <div class="flex-1 flex flex-wrap items-center filter-options">
            <a href="javascript:;" @click="selectedNewTime = ''" :class="['text-xs px-2 py-1 rounded transition cursor-pointer filter-option', !selectedNewTime ? 'bg-primary text-white' : 'text-slate-700 hover:text-primary']">{{ t('all') }}</a>
            <a v-for="option in newTimeOptions" :key="option.value" v-show="option.value" href="javascript:;" @click="selectedNewTime = selectedNewTime === option.value ? '' : option.value" :class="['text-xs px-2 py-1 rounded transition cursor-pointer filter-option', selectedNewTime === option.value ? 'bg-primary text-white' : 'text-slate-700 hover:text-primary']">{{ option.label }}</a>
          </div>
        </div>
        ...
        <div class="flex items-center px-5 py-2 border-b border-slate-200">
          ...
          <div class="flex-1 flex flex-wrap items-center filter-options">
            <a href="javascript:;" @click="selectedShipment = ''" :class="['text-xs px-2 py-1 rounded transition cursor-pointer filter-option', !selectedShipment ? 'bg-primary text-white' : 'text-slate-700 hover:text-primary']">{{ t('all') }}</a>
            <a v-for="option in shipmentOptions" :key="option.value" v-show="option.value" href="javascript:;" @click="selectedShipment = selectedShipment === option.value ? '' : option.value" :class="['text-xs px-2 py-1 rounded transition cursor-pointer filter-option', selectedShipment === option.value ? 'bg-primary text-white' : 'text-slate-700 hover:text-primary']">{{ option.label }}</a>
          </div>
        </div>
        ...
        <div class="flex items-center px-5 py-2 border-b border-slate-200">
          ...
          <div class="flex-1 flex flex-wrap items-center filter-options">
            <a href="javascript:;" @click="selectedSaleMode = ''" :class="['text-xs px-2 py-1 rounded transition cursor-pointer filter-option', !selectedSaleMode ? 'bg-primary text-white' : 'text-slate-700 hover:text-primary']">{{ t('all') }}</a>
            <a v-for="option in saleModeOptions" :key="option.value" v-show="option.value" href="javascript:;" @click="selectedSaleMode = selectedSaleMode === option.value ? '' : option.value" :class="['text-xs px-2 py-1 rounded transition cursor-pointer filter-option', selectedSaleMode === option.value ? 'bg-primary text-white' : 'text-slate-700 hover:text-primary']">{{ option.label }}</a>
          </div>
        </div>
      ...
      <div class="bg-white shadow-sm mb-6">
        <div class="flex items-center px-5 py-3 border-b border-slate-200">
          ...
          <div class="flex-shrink-0">
            <ul class="flex items-center select-actions">
              <li class="flex items-center select-all-item">
                <input type="checkbox" v-model="selectAll" @change="handleSelectAll" class="rounded cursor-pointer" />
                <span class="text-sm font-medium text-slate-700 cursor-pointer select-all-text">{{ t('selectAll') }}</span>
              </li>
              <li>
          ...
          <div class="flex-shrink-0">
            <ul class="flex items-center batch-actions">
              <li><a href="javascript:;" class="text-xs text-slate-700 hover:text-primary transition inline-flex items-center action-link" :title="t('publish')">
                <img src="/frondend/images/AllCategoriesProductsPage/publish_icon.png" :alt="t('publish')" class="w-4 h-4 action-icon" />
                {{ t('publish') }}
              </a></li>
              <li><a href="javascript:;" @click="batchDownloadProducts" :class="{'opacity-50 cursor-not-allowed': batchDownloading}" class="text-xs text-slate-700 hover:text-primary transition inline-flex items-center action-link">
                <svg v-if="batchDownloading" class="w-4 h-4 animate-spin action-icon" fill="none" viewBox="0 0 24 24">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                  <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#cb261c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="action-icon">
              ...
              <li><a href="javascript:;" @click="handleBatchFavorite" class="text-xs text-slate-700 hover:text-primary transition inline-flex items-center action-link">
                <!-- 已收藏：实心红心，不设置 stroke -->
                <svg v-if="batchFavoriteIconFilled" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="#cb261c" class="action-icon">
              ...
              <li><a href="javascript:;" class="text-xs text-slate-700 hover:text-primary transition inline-flex items-center action-link">
                <DownloadIcon class="w-4 h-4 text-[#2F54EB] action-icon" />
                {{ t('downloadSpu') }}
              </a></li>
              <li><a href="javascript:;" class="text-xs text-slate-700 hover:text-primary transition inline-flex items-center action-link">
                <CopyIcon class="w-4 h-4 text-[#13C2C2] action-icon" />
                {{ t('copySpu') }}
              </a></li>
            </ul>
          </div>
        ...
        <div class="product-grid mb-8">
          ...
          >
            ...
            <a :href="`/shop/item/${product.id}`" target="_blank" rel="noopener noreferrer" class="block aspect-square overflow-hidden bg-slate-100 relative">
              <!-- 折扣标签（商品右上角） -->
              <div v-if="getCurrentDiscount(product) > 0" class="absolute top-2 right-2 bg-yellow-400 text-black text-xs font-bold px-2 py-1 rounded z-10">
            ...
            <div class="p-4">
              ...
              <!-- 价格显示 -->
              <div class="mb-2 p-2 bg-slate-50 rounded text-center price-box">
                <!-- 有折扣时显示折扣价 -->
                <template v-if="getCurrentDiscount(product) > 0">
                  <div class="text-red-500 font-bold text-lg">
                    {{ formatPrice(calculateDiscountedPrice(product), getCurrentCurrency(product)) }}
                  </div>
                  <div class="text-gray-500 text-sm line-through">
                    {{ formatPrice(getCurrentSellingPrice(product), getCurrentCurrency(product)) }}
                  </div>
                </template>
                <!-- 无折扣时只显示售价 -->
                <template v-else>
              ...
              <!-- 区域选择器（价格下方） -->
              <div class="mb-2 flex items-center justify-center flex-wrap region-selector" v-if="product.shippingRegions && product.shippingRegions.length > 0">
                <button
              ...
              <div class="flex items-center justify-between pt-3">
                <label class="flex items-center cursor-pointer product-checkbox">
                  <input 
                    type="checkbox" 
                    class="rounded" 
                ...
                <div class="flex items-center product-actions">
...
const loadLanguageFile = async () => {
  try {
    const response = await fetch('/frondend/lang/crossBordereCommercePage.json')
    const data = await response.json()
    messages.value = data
  } catch (error) {
    console.error('Failed to load language file / 加载多语言文件失败:', error)
    // 如果加载失败，使用默认的空对象
    messages.value = {}
  }
}
...
// 翻译函数
const t = (key, params = {}) => {
  let text = messages.value[currentLang.value]?.[key] || messages.value['zh-CN']?.[key] || key
  // 替换占位符 {key} 为实际值
...
const fetchProducts = async (page = 1) => {
  try {
    ...
    console.error(t('fetchProductsFailed') + ':', error)
    isLoading.value = false
  }
}
...
// 下载状态管理
const downloadedProductsSet = ref(new Set())
...
const newTimeOptions = computed(() => [
  { label: t('all'), value: '' },
  { label: t('newTimeDays', { days: 7 }), value: '7' },
  { label: t('newTimeDays', { days: 15 }), value: '15' },
  { label: t('newTimeDays', { days: 30 }), value: '30' },
  { label: t('newTimeDays', { days: 60 }), value: '60' },
])
...
const fetchDownloadStats = async () => {
  try {
    ...
    console.error(t('fetchDownloadQuotaFailed') + ':', error)
  }
}
  ...
  try {
    ...
    if (downloadedProductIds.length > 0) {
      ...
      if (downloadedProductIds.length > 5) {
        message += t('andMoreProducts', { count: downloadedProductIds.length }) + '\n'
      }
    ...
    try {
      ...
      if (failCount === 0) {
        ...
        ElMessage.warning(t('downloadResultMessage', { successCount, failCount }))
      }
...
const createNewGroup = async () => {
  ...
  try {
    ...
    console.error(t('createGroupFailed') + ':', error)
...
const addSingleFavorite = async (productId, groupId) => {
  try {
    ...
    console.error(t('addFavoriteFailed') + ':', error)
...
const removeFavorite = async (productId, favoriteId) => {
  try {
    ...
    console.error(t('removeFavoriteFailed') + ':', error)
...
const addBatchFavorite = async (productIds, groupId) => {
  try {
    ...
    console.error(t('batchFavoriteFailed') + ':', error)
...
<style scoped>
/* ===== 兼容性修复：使用 margin 替代 gap ===== */

/* 已选条件标签 */
.selected-conditions > *:not(:last-child) {
  margin-right: 8px;
}

.condition-tag > *:not(:last-child) {
  margin-right: 4px;
}
...
.select-all-item > *:not(:last-child) {
  margin-right: 8px;
}

/* 批量操作 */
.batch-actions > *:not(:last-child) {
  margin-right: 24px;
}

.action-link > .action-icon:not(:last-child) {
  margin-right: 4px;
}
...
/* 区域选择器 */
.region-selector > *:not(:last-child) {
  margin-right: 4px;
}
...
.sort-link {
  ...
  padding:0 20px;line-height:36px;color:#333;text-decoration:none;border-right:1px solid #ddd;cursor:pointer
}
...
.btn-create-group > *:not(:last-child) {
  margin-right: 8px;
}
...
.create-group-form > *:not(:last-child) {
  margin-bottom: 20px;
}
